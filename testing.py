...
class ChoiceFour(tk.Toplevel):
    ...
    def checkValid(self):
        ...
        if len(self.LB.curselection()) <= 0:
            ...
            for index in curSelected:
                resWindow = ShowRestaurantsWindow(self,self.data,index)
                threads.append(threading.Thread(target=self.searchAPI,args=(self.data,index)))
            ...
            for t in threads:
                ...
                # resWindow.statusText.set("Finished " + str(n) + "/" + str(len(threads)) + " searches.")
                self.update()                               # Updates GUI to reflect # of searches completed
                # self.results = {**self.results, **data}     # Merges existing results and new data into new results dict
    ...
    def searchAPI(self, selectedRestaurant,index):
        """Threaded method to search API for food item and put it in the queue

        Arguments:
            query (string): food item to lookup in API
        """
        response = genSearch(selectedRestaurant['locations'][index]['name'], BASE_URL, HEADERS)
        self.queue.put(response)
...
class ShowRestaurantsWindow(tk.Toplevel):
    def __init__(self, master, restaurant, i):
        ...
        self.statusText.set("")
        self.status = tk.Label(self, textvariable=self.statusText).grid(padx=10, pady=5)
